...
def descobrir_arquivos_json():
    """Descobre automaticamente todos os arquivos JSON de faltas disponíveis"""
    ...
    if os.path.exists(jsons_dir):
        ...
        for arquivo in found_files:
            if 'falta' in os.path.basename(arquivo):
                json_files.append(arquivo)
        
        json_files = sorted(json_files)
        logging.info(f"Arquivos JSON de faltas descobertos automaticamente: {len(json_files)} arquivos")
            ...
            for entry in record:
                for item_data in entry['data']:
                    item = {
                        ...
                        'Abono': item_data.get('missing_type', ''),
                        'Motivo': item_data.get('missing_motive', '')
                    }
                    
                    turno = item_data.get('shift', '')
                    if turno:
                        cod_turno, desc_turno = turno.split(' - ', 1) if ' - ' in turno else (turno, '')
                        item['CodTurno'] = cod_turno.strip()
                        item['DescTurno'] = desc_turno.strip()
                    else:
                        item['CodTurno'] = ''
                        item['DescTurno'] = ''
...
def main():
    ...
    logging.info(f"Iniciando processamento do falta_treat.py às {start_time}")
    ...
    try:
        ...
        if not SKIP_DROP:
            ...
            CREATE TABLE [{table_name}] (
                ...
                [Cargo] NVARCHAR(500),
                [Equipe] NVARCHAR(500),
                [CodTurno] NVARCHAR(255),
                [DescTurno] NVARCHAR(500),
                [Data] NVARCHAR(255),
                [Abono] NVARCHAR(MAX),
                [Motivo] NVARCHAR(MAX)
            )
        ...
        INSERT INTO [{table_name}] (
            [Nome], [Matricula], [Cargo], [Equipe], [CodTurno], [DescTurno], [Data], [Abono], [Motivo]
        ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)
        """
        ...
        for json_file in tqdm(json_files, desc="Processando e Inserindo"):
            ...
            if data_list:
                ...
                df_temp = df_temp[['Nome', 'Matricula', 'Cargo', 'Equipe', 'CodTurno', 'DescTurno', 'Data', 'Abono', 'Motivo']]
