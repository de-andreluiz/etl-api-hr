...
OUTPUT_FILE = os.path.join(BASE_DIR, "falta01-03.json")
...
body = {
    "report": {
        ...
        "group_by": "team",
        "row_filters": "",
        "columns": "name,registration_number,job_title,team,shift,date,missing_type,missing_motive",
        "format": "json"
    }
}
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            error_message = (f"Erro na requisição: {response.status_code}. Detalhes: {response.text} "
                           f"Arquivo: {os.path.basename(__file__)}, Hora: {error_time}, "
