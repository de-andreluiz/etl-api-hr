...
def descobrir_arquivos_json():
    """Descobre automaticamente todos os arquivos JSON de afastamento disponíveis"""
    ...
    if os.path.exists(jsons_dir):
        ...
        for arquivo in found_files:
            if 'afastamento' in os.path.basename(arquivo):
                json_files.append(arquivo)
        
        json_files = sorted(json_files)
        logging.info(f"Arquivos JSON de afastamento descobertos automaticamente: {len(json_files)} arquivos")
            ...
            for entry in record:
                for item_data in entry['data']:
                    item = {
                        ...
                        'DtInicio': item_data.get('start_date', '').split(',')[-1].strip(),
                        'DtTermino': item_data.get('end_date', '').split(',')[-1].strip(),
                        'Notas': item_data.get('observation', '')
                    }
...
def main():
    ...
    logging.info(f"Iniciando processamento do afastamento_treat.py às {start_time}")
    ...
    try:
        ...
        INSERT INTO [{table_name}] (
            [Nome], [Matricula], [Equipe], [DtInicio], [DtTermino], [Notas]
        ) VALUES (?, ?, ?, ?, ?, ?)
        """
        ...
        for json_file in tqdm(json_files, desc="Processando e Inserindo"):
            ...
            if data_list:
                ...
                df_temp = df_temp[['Nome', 'Matricula', 'Equipe', 'DtInicio', 'DtTermino', 'Notas']]
