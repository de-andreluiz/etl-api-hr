...
json_file = os.path.join(BASE_DIR, "departamentos_register.json")
...
def main():
    ...
    logging.info(f"Iniciando processamento do departamentos_treat.py às {start_time}")
    ...
    try:
        ...
        if not SKIP_DROP:
            ...
            CREATE TABLE [{table_name}] (
                ...
                [CodUnidadeNegocio] NVARCHAR(255),
                [NomeUnidadeNegocio] NVARCHAR(255),
                [ContagemFuncionarios] NVARCHAR(255)
            )
            """
        ...
        for department in departments:
            business_unit = department.get('business_unit', {})
            item = {
                'Codigo': department.get('code', ''),
                'Nome': department.get('name', ''),
                'Observacao': department.get('observation', ''),
                'CodUnidadeNegocio': business_unit.get('code', ''),
                'NomeUnidadeNegocio': business_unit.get('name', ''),
                'ContagemFuncionarios': department.get('employees_count', '')
            }
        ...
        if normalized_data:
            ...
            df = df[['Codigo', 'Nome', 'Observacao', 'CodUnidadeNegocio', 'NomeUnidadeNegocio', 'ContagemFuncionarios']]
            df = df.fillna('').astype(str)

            insert_sql = f"""
            INSERT INTO [{table_name}] (
                [Codigo], [Nome], [Observacao], [CodUnidadeNegocio], [NomeUnidadeNegocio], [ContagemFuncionarios]
            ) VALUES (?, ?, ?, ?, ?, ?)
            """
