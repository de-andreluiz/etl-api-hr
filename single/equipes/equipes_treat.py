...
json_file = os.path.join(BASE_DIR, "equipes_register.json")
...
def main():
    ...
    logging.info(f"Iniciando processamento do equipes_treat.py às {start_time}")
    ...
    try:
        ...
        if not SKIP_DROP:
            ...
            CREATE TABLE [{table_name}] (
                ...
                [Gestor1] NVARCHAR(255),
                [Gestor2] NVARCHAR(255),
                [Gestor3] NVARCHAR(255),
                [Gestor4] NVARCHAR(255),
                [Gestor5] NVARCHAR(255)
            )
            """
        ...
        if normalized_data:
            ...
            df = df[['Nome', 'UnidadeNegocio', 'Gestor1', 'Gestor2', 'Gestor3', 'Gestor4', 'Gestor5']]
            df = df.fillna('').astype(str)

            insert_sql = f"""
            INSERT INTO [{table_name}] (
                [Nome], [UnidadeNegocio], [Gestor1], [Gestor2], [Gestor3], [Gestor4], [Gestor5]
            ) VALUES (?, ?, ?, ?, ?, ?, ?)
            """
