...
json_file = os.path.join(BASE_DIR, "feriados_register.json")
...
def main():
    ...
    logging.info(f"Iniciando processamento do feriados_treat.py às {start_time}")
    ...
    try:
        ...
        if not SKIP_DROP:
            ...
            CREATE TABLE [{table_name}] (
                ...
                [Fixo] NVARCHAR(255),
                [Data] NVARCHAR(255),
                [CodEquipe] NVARCHAR(255),
                [DescEquipe] NVARCHAR(255),
                [CodDepto] NVARCHAR(255),
                [DescDepto] NVARCHAR(255),
                [CodUn] NVARCHAR(255),
                [DescUn] NVARCHAR(255),
                [CodCC] NVARCHAR(255),
                [DescCC] NVARCHAR(255),
        ...
        if 'holidays' not in json_data:
            raise KeyError("A chave 'holidays' não foi encontrada no JSON.")

        holidays = json_data['holidays']
        normalized_data = []

        for holiday in holidays:
            if holiday is None:
                logging.warning("Encontrado um item None na lista de feriados, pulando...")
                continue

            date_iso = holiday.get('date', '')
            date_formatted = datetime.strptime(date_iso, '%Y-%m-%d').strftime('%d/%m/%Y') if date_iso else ''

            team = holiday.get('team') if holiday.get('team') is not None else {}
            department = holiday.get('department') if holiday.get('department') is not None else {}
            business_unit = holiday.get('business_unit') if holiday.get('business_unit') is not None else {}
            cost_center = holiday.get('cost_center') if holiday.get('cost_center') is not None else {}

            item = {
                'ID': str(holiday.get('id', '')),
                'Nome': holiday.get('name', ''),
                'Fixo': str(holiday.get('fixed', '')),
                'Data': date_formatted,
                'CodEquipe': team.get('code', ''),
                'DescEquipe': team.get('name', ''),
                'CodDepto': department.get('code', ''),
                'DescDepto': department.get('name', ''),
                'CodUn': business_unit.get('code', ''),
                'DescUn': business_unit.get('name', ''),
                'CodCC': cost_center.get('code', ''),
                'DescCC': cost_center.get('name', ''),
                'Turno': holiday.get('shift', '') or ''
            }
        ...
        if normalized_data:
            ...
            df = df[['ID', 'Nome', 'Fixo', 'Data', 'CodEquipe', 'DescEquipe', 'CodDepto', 'DescDepto', 'CodUn', 'DescUn', 'CodCC', 'DescCC', 'Turno']]
            df = df.fillna('').astype(str)

            insert_sql = f"""
            INSERT INTO [{table_name}] (
                [ID], [Nome], [Fixo], [Data], [CodEquipe], [DescEquipe], [CodDepto], [DescDepto], [CodUn], [DescUn], [CodCC], [DescCC], [Turno]
            ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
            """
